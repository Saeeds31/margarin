...
import { BModal, BButton, BOverlay, BSpinner,BFormInput,BFormGroup,BTab,BTabs,BForm } from "bootstrap-vue";
export default {
  components: { SInputs, BModal, BButton, STable, BOverlay, BSpinner,BFormInput,BFormGroup,BTab,BTabs ,BForm},
  data() {
    return {
      disabled: false,
      headers: [
  ...
  methods: {
    loadItems() {
      this.$axios
        .get("ProductFeature/GetProductFeature?ProoductId=" + this.productId)
        .then((resp) => {
    ...
    setErrorResponse(error) {
     ...
     submit() {
      ...
      if (pack.mode == "edit") {
        ...
        pack.data.productId=this.productId
