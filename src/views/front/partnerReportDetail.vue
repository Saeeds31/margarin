<template>
  <main v v-if="blogData" id="blogDetailSection" class="magazineDatail width80 margin-auto">
    <introduction
       :title="
       $cookie.get('ltrTheme')?'Internal news':'اخبار داخلی'"
...
export default {
  ...
  computed: {
    ...
    blogData() {
    return  this.$store.getters.getSinglePartner;
    }
  },
  watch: {
    '$route.params.id':{
      handler(){
      // this.$store.dispatch("getPartnerSingleFromServer", this.$route.params.id);
      this.checkRequest('getPartnerSingleFromServer',JSON.stringify(this.$route.params.id));
