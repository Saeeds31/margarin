<template>
  <main v-if="blogsData != null" id="blogsSection" class="width80 margin-auto">
    <introduction
      :title="getTitle()"
      :summary="blogsData.data.reportageIntro.title"
      :image="$root.baseImageUrl + blogsData.data.reportageIntro.image"
      :routes="routes"
    >
      <p class="slotElements">
        {{ blogsData.data.reportageIntro.text }}
      </p>
    </introduction>
...
import weblogCart from "@/components/front/reportage/weblogCart.vue";
...
export default {
    ...
    "$route.query": {
      handler(value) {
        ...
        let pack = {
          ...
          // this.$store.dispatch("getReportageFromServer", pack);
      this.checkRequest('getReportageFromServer',JSON.stringify(pack));
  ...
  metaInfo() {
    return {
      ...
      meta: [
        {
          name: "description",
          content: this.blogsData ? this.blogsData.data.reportageIntro.text : false
        },
        {
          property: "og:title",
  ...
  computed: {
    blogsData() {
      return this.$store.getters.getReportageData;
    }
  },
  mounted() {
  ...
  methods: {
    getTitle() {
        if (this.$cookie.get("ltrTheme")) return "reportage Magazine";
        else return "گزارشات ";
