<template>
  <main v v-if="blogData" id="blogDetailSection" class="magazineDatail width80 margin-auto">
    <introduction
       :title="
       $cookie.get('ltrTheme')?'magazine aftab':'مجله آفتاب'"
...
export default {
  ...
  computed: {
    ...
    blogData() {
    return  this.$store.getters.getSingleMagazine;
    }
  },
  watch: {
    '$route.params.id':{
      handler(){
      // this.$store.dispatch("getMagazineSingle", this.$route.params.id);
      this.checkRequest('getMagazineSingle',JSON.stringify(this.$route.params.id));
