<template>
  <main v v-if="blogData" id="blogDetailSection" class="magazineDatail width80 margin-auto">
    <introduction
       :title="
       $cookie.get('ltrTheme')?'magazine aftab':'گزارشات'"
      :summary="$cookie.get('ltrTheme')?'Follow us ':' با ما همراه باشد'"
...
export default {
  ...
  computed: {
    ...
    blogData() {
    return  this.$store.getters.getSingleReportage;
    }
  },
  watch: {
    '$route.params.id':{
      handler(){
      // this.$store.dispatch("getReportageSingleFromServer", this.$route.params.id);
      this.checkRequest('getReportageSingleFromServer',JSON.stringify(this.$route.params.id));
